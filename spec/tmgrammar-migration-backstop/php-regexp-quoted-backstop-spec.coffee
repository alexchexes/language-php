...
describe 'PHP quoted regexp tmgrammar migration backstop', ->
  ...
  describe 'quoted assertion groups', ->
  ...
  describe 'quoted conditionals', ->
    ...
    it 'should tokenize the supported assertion-conditional families in quoted regexes', ->
      expectedConditions = [
  ...
  describe 'quoted backtracking verbs', ->
  ...
  describe 'quoted group boundaries', ->
  ...
  describe 'quoted host quote parity', ->
    ...
    it 'should keep opposite-host quotes inside quoted regex character classes after even interpreted parity', ->
      ...
      expect(singleQuotedOppositeQuote.tokens[5]).toEqual value: '"', scopes: [
        'source.php'
        'meta.embedded.regexp.php'
        'string.regexp.single-quoted.php'
        'meta.embedded.character-class.regexp.php'
      ...
      expect(doubleQuotedOppositeQuote.tokens[5]).toEqual value: '\'', scopes: [
        'source.php'
        'meta.embedded.regexp.php'
  ...
  describe 'quoted short hex escapes', ->
...
describe 'PHP quoted regexp body numeric migration backstop', ->
  grammar = null
  before(-> grammar = await loadGrammar('source.php'))
...
describe 'PHP quoted regexp transport migration backstop', ->
  grammar = null
  before(-> grammar = await loadGrammar('source.php'))
  ...
  it 'should keep transported PHP octal and simple escapes PHP-first in double quoted regex bodies', ->
    ...
    {tokens} = grammar.tokenizeLine '"/' + '\\'.repeat(3) + '1' + basicEscapes.map((char) -> '\\'.repeat(3) + char).join('') + '/"'
    ...
    for char in basicEscapes
      expect(tokens[offset]).toEqual value: '\\\\', scopes: quotedDoubleRegexpScope.concat ['constant.character.escape.php']
      expect(tokens[offset + 1]).toEqual value: '\\' + char, scopes: quotedDoubleRegexpScope.concat ['constant.character.escape.php']
      offset += 2
    expect(tokens[offset]).toEqual value: '/', scopes: regexpWrapperEndDelimiterScopes(quotedDoubleRegexpScope)
    expect(tokens[offset + 1]).toEqual value: '"', scopes: regexpWrapperEndQuoteScopes(quotedDoubleRegexpScope)
